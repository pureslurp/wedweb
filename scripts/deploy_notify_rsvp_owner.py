#!/usr/bin/env python3
"""Deploy the notify-rsvp-owner Edge Function via Supabase Management API.
...
Usage:
  ...
  python3 scripts/deploy_notify_rsvp_owner.py
...
FN_FILE = ROOT / "supabase/functions/notify-rsvp-owner/index.ts"
    ...
    metadata = json.dumps(
        {
            ...
            "name": "notify-rsvp-owner",
            "verify_jwt": False,
        },
        separators=(",", ":"),
    )

    url = f"https://api.supabase.com/v1/projects/{ref}/functions/deploy?slug=notify-rsvp-owner"
    cmd = [
        "curl",
        "-sS",
        "-w",
